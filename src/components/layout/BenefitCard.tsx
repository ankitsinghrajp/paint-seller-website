...
const AnimatedIcon = ({ IconComponent, isVisible, delay = 0 }) => {
  ...
  return (
    <div 
      className={`relative w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-yellow-400 via-orange-400 to-yellow-600 flex items-center justify-center group cursor-pointer transition-all duration-700 ${
    ...
    >
      {/* Outer glow effect */}
      <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br from-yellow-400 to-orange-600 transition-all duration-500 ${
      ...
      {/* Icon */}
      <IconComponent className={`h-9 w-9 text-gray-800 relative z-10 transition-all duration-300 ${
...
const BenefitCard = ({ benefit, index, isVisible }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 -m-4 rounded-3xl bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm transition-all duration-500 ${
      ...
      <div className="relative z-10 p-4">
        ...
        <h3 className={`text-2xl font-bold text-white mb-4 transition-all duration-300 ${
          isHovered ? 'text-yellow-300 scale-105' : ''
        }`}>
          {benefit.title}
        </h3>
        
        <p className={`text-white/80 leading-relaxed transition-all duration-300 ${
          isHovered ? 'text-white/95 scale-[1.02]' : ''
        }`}>
          {benefit.description}
        </p>
        
        {/* Hover indicator */}
        <div className={`mt-4 h-1 bg-gradient-to-r from-yellow-400 to-orange-400 rounded-full transition-all duration-500 ${
      ...
      <div className={`absolute top-0 left-0 w-full h-full overflow-hidden rounded-3xl transition-opacity duration-500 ${
        ...
        <div className="absolute top-4 left-4 w-2 h-2 bg-yellow-400/30 rounded-full animate-float" style={{ animationDelay: '0s' }}></div>
        <div className="absolute top-8 right-6 w-1.5 h-1.5 bg-orange-400/30 rounded-full animate-float" style={{ animationDelay: '1s' }}></div>
        <div className="absolute bottom-6 left-8 w-1 h-1 bg-yellow-300/30 rounded-full animate-float" style={{ animationDelay: '2s' }}></div>
      </div>
    </div>
  );
};
...
const PremiumPaintBenefits = () => {
  ...
  return (
    <section 
      ref={sectionRef}
      className="py-20 bg-gradient-to-br from-gray-900 via-blue-900 to-gray-800 relative overflow-hidden"
    >
      {/* Animated background elements */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 left-10 w-64 h-64 bg-yellow-400 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '0s' }}></div>
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-orange-400 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 left-1/2 w-48 h-48 bg-blue-400 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '4s' }}></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ${
          ...
          <div className="inline-flex items-center justify-center p-3 bg-gradient-to-r from-yellow-400 to-orange-400 rounded-full mb-6">
            <Award className="h-6 w-6 text-gray-800 mr-2" />
            <span className="text-gray-800 font-bold text-sm uppercase tracking-wide">Premium Quality</span>
          </div>
          ...
          <p className="text-xl text-white/80 max-w-3xl mx-auto leading-relaxed">
            ...
            <span className="font-semibold text-yellow-300"> carefully curated selection</span>
          </p>
        </div>
